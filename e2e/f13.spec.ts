...
/** F13 — step 2 original style: textured swatches, legible icons, hover preview. */

const B1 = "/no/configurator?design=blomster-1&step=2";
const AMALFI = "/no/configurator?design=amalfi-dyr&step=2";

test("AC1: swatches carry the glaze texture overlays (not a flat disc)", async ({
  page,
}) => {
  await page.goto(B1);
  const swatch = page
  ...
  // two shared overlay spans (multiply grain + screen speckle) over the hex
  const overlays = await swatch.evaluate((el) => {
    const spans = Array.from(el.querySelectorAll("span"));
    return spans.map((s) => getComputedStyle(s).mixBlendMode);
  });
  expect(overlays).toContain("multiply");
  expect(overlays).toContain("screen");
});

test("AC2: kind=image icons are monochrome silhouettes (mask + currentColor)", async ({
  page,
}) => {
  ...
  expect(style.mask).toContain("url("); // masked by the asset
  expect(style.bg).not.toBe("rgba(0, 0, 0, 0)"); // filled with currentColor
});
