...
**QA-fixes — batch su `qa-fix-bugs`** (giro bug post back-office, 2026-06-09). Piccole fix raccolte su un unico branch:
- **#1 — carrello non si svuota dopo l'invio ordine** ✓ fix applicata. Causa: persistenza `localStorage` via `useEffect` differito; `order-form` fa `clear()` poi `router.push('/order')` subito → la navigazione smonta prima che l'effetto scriva `[]` → ri-idratazione col carrello pieno. Fix: `clear()` in `use-cart.ts` scrive `[]` su `localStorage` **sincrono**. TODO: e2e "invio → carrello vuoto anche dopo navigazione". (Bug Krabbe "overlayered" = risolto da Daniele lato dato, no codice.)
- **#2 — step 3: via il "Send bestilling ›" disabilitato (UI morta) + CTA "Lag nytt design"**. In alto a destra sullo step 3 il bottone nav è disabilitato/inutile (il vero invio è nel carrello) → **rimuoverlo**. Dopo "Legg i handlekurv" (dove ora c'è "Lagt i handlekurven") aggiungere un CTA **"Lag et nytt design →"** che riporta allo **step 1 mantenendo il design scelto** (config/URL preservata) per comporre un'altra variante; **carrello intatto** (persistente). Opzionale: un discreto "+ Nytt design" nello slot top-right dello step 3 al posto del bottone morto. FE: nav cluster (`configurator-client`) + area add-to-cart (`ceramics-step`). Invita a continuare gli acquisti → multi-pezzo.
- **#3 — mobile: preview "flippa" (collapse a thumbnail in loop)** ✓ fix applicata (**soluzione B**). Causa: `IntersectionObserver threshold:0` senza isteresi + churn della barra indirizzi mobile (il viewport cambia altezza scrollando) → collasso/espansione oscillano all'infinito; visibile solo su mobile (lo stile collapse è `max-md:*`). Fix in `configurator-client.tsx`: rimosso IO + sentinel + toggle width → su mobile la preview scorre normalmente; desktop `md:sticky` invariato. Ripuliti import `useRef`/`useState`; e2e `f15` AC5 aggiornato (asserisce assenza di `data-collapsed`). **TODO: giro QA mobile dedicato** (mobile mai testata davvero) → eventuale sticky-thumbnail, testato, dopo.
- *(prossime fix qui)*
