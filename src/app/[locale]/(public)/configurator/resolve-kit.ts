...
export interface ResolvedKit {
  ...
  imageCustom: boolean;
  /** the featured label of this exact kit (raw label_no/label_en — the card
   *  fallback stays the strip's job). Null: no featured match. */
  label: { no: string | null; en: string | null } | null;
}
...
export async function resolveKit(raw: string): Promise<ResolvedKit> {
  ...
  return {
    ...
    label: featured
      ? { no: featured.labelNo, en: featured.labelEn }
      : null,
  };
}
