...
import { mapAttributes, type ProductAttribute } from "@/lib/catalog/product-attributes";
...
export interface SupplierProduct {
  ...
  /** R2-4: ordered free-form attributes (dimensions, weight…); [] when none. */
...
async function loadSupplierProducts(
  ...
  const { data, error } = await supabase
    .from("products")
    .select(
      "id, slug, name_no, name_en, description_no, description_en, price_cents, currency, image, pieces, product_attributes(label_no, label_en, value, sort_order)"
    )
  ...
  return (data ?? []).map((p) => ({
    ...
    attributes: mapAttributes(p.product_attributes),
  }));
}
